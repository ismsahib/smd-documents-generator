...
const Instruction: FC<InstructionProps> = ({ modalShow, setModalShow }) => (
  ...
  >
    ...
    <Modal.Body className={styles.myModalBody}>
      ...
      <ol>
        <li>
          1) для текстовых значений <b>{`{<variables-столбец-строка>}`}</b> или{" "}
          <b> {`{&lt;variables-столбец-строка&gt;}`}</b>
        </li>
        <li>
      ...
      <div>
        <i>Таблицы</i>
        <br />
        Текст в таблицу вставляется по-умолчанию шрифтом Times New Roman 12pt с маркером желтого цвета, без пустых строк
        и столбцов. Изображения уменьшаются до нормы (100px на 100px).
      </div>
      <div>
        <i>Диапазон строк</i>
        <br />
        Текст вставляется по-умолчанию шрифтом Times New Roman 12pt с маркером желтого цвета. Изображения уменьшаются до
        нормы (100px на 100px).
      </div>
      <div>
        <i>Значения из других таблиц</i>
        <br />
        После указания позиции переменной в импортируемой таблице необходимо указать <b>@import(столбец,строка)</b>, где
        <b>(столбец,строка)</b> - позиция ячейки со ссылкой на импортируемую таблицу. Пример:{" "}
        <b>{`<variables-2-1@import(1,50)>`}</b>
      </div>
    </Modal.Body>
    <Modal.Footer>
